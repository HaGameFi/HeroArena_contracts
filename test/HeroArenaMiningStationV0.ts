...
describe("HeroArenaMiningStationV0", async function () {
  ...
  async function deploy() {
    ...
    const station  = await viem.deployContract("HeroArenaMiningStationV0", [
      hapToken.address,
      NFT_PRICE,
    ]);
  ...
  describe("constructor", async function () {
    ...
    it("sets frame name in constructor", async function () {
      const { framesSC } = await deploy();
      const [names] = await framesSC.read.getFrameNameAndCreatedTimestampBatch([[0]]);
  ...
  describe("mintNFT", async function () {
    ...
    it("increments frameCount", async function () {
      ...
      await station.write.updateAvailableClaim([true]);
      await station.write.mintNFT([0], { account: user1Client.account });
      assert.equal(await framesSC.read.frameCount([0]), 1n);
    });
    ...
    it("multiple users can mint", async function () {
      ...
      await station.write.updateAvailableClaim([true]);
      await station.write.mintNFT([0], { account: user1Client.account });
      await station.write.mintNFT([0], { account: user2Client.account });
      assert.equal(await framesSC.read.balanceOf([user1]), 1n);
    ...
    it("reverts on invalid frameId", async function () {
      ...
      await station.write.updateAvailableClaim([true]);
      await assert.rejects(
        station.write.mintNFT([1], { account: user1Client.account }),
